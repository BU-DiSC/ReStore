...
int main(
    ...
    std::ofstream logFile("output_20hf90_rw1_10000_LFU.log");
    ...
    std::ofstream outTier("Tier123_20hf90_rw1_10000_LFU.txt");
    ...
    for (int i = 0; i < total_num_reqs; ++i){
        ...
        if (it1 != Tier1.end()) {
            ...
            // Compare the frequency of current page with all pages in Tier1
            int n_smaller = 0;   // number of pages in Tier1 which has smaller frequency than current page
            for (const auto& pair : Tier1) {
                int key = pair.first;
                if (list_num_req[key] < list_num_req[n_page]){
                    ++n_smaller;
                }
            }
            // If current page has larger frequency than any page in Tier1, then upgrade current page to Tier1
            if (n_smaller > 2){  // 0 is a hyperparameter, adapted to k of LFU-k
            ...
            // Compare the frequency of current page with all pages in Tier2
            int n_smaller = 0;   // number of pages in Tier2 which has smaller frequency than current page
            ...
            // If current page has larger frequency than any page in Tier2, then upgrade current page to Tier1
            if (n_smaller > 8){  // 8 is a hyperparameter, adapted to k of LFU-k
