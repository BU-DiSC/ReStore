...
int main(
    ...
    std::string log_path = folder_name + "/output_" + workload + "_LFU.log";
    ...
    // std::string outtier_path = folder_name + "Tier123_" + workload_path + "_LFU.txt";
    ...
    LFUCache LC_T1(max_capacity_tier1);
    LFUCache LC_T2(max_capacity_tier2);
    LFUCache LC_T3(max_capacity_tier3);
    // create reference for page_map
    ...
    if (workload == "YCSB" || workload == "TPCC" || workload == "TPCE" || workload.find("MSR") != std::string::npos) {
        // initial policy for these workloads: fill with specific page ids
    ...
    // Counter of runtime
    long long total_runtime_LFU = 0; // Variable to accumulate durations
    ...
    while (std::getline(file, line)) {
        ...
        if (it1 != Tier1.end()) {
            ...
            // Update page's frequency
            LC_T2.requestPage(n_page);

            // Decide migration by LFU
            auto start_time_LFU = std::chrono::high_resolution_clock::now();
            // Compare the frequency of current page with LFU pages in Tier1
            Page current_page = LC_T2.page_map[n_page];
            Page lfu_page_t1  = LC_T1.min_heap.top();
            // skip non_existed page or non-matched LFU page
            while ((Tier1.find(lfu_page_t1.id) == Tier1.end()) ||
                   (lfu_page_t1.frequency != LC_T1.page_map[lfu_page_t1.id].frequency)) {
                LC_T1.min_heap.pop();
                lfu_page_t1  = LC_T1.min_heap.top();
            }
            // If current page has larger frequency than any page in Tier1, then upgrade current page to Tier1
            // std::cout << "Current page's frequency: " << current_page.frequency << std::endl;
            // std::cout << "LFU page in Tier1 frequency: " << lfu_page_t1.frequency << std::endl;
            if (current_page.frequency > lfu_page_t1.frequency){
                ++num_migr_t2t1;
                ...
                while (Tier1.size() > max_capacity_tier1){
                    ...
                    LC_T1.evictPage(lfu_page_t1);
                    LC_T2.addPage(lfu_page_t1);
                    // new task should be submitted here to simulate page downgrade
                    // read from Tier1
                    tier1_dr.exec(lfu_page_t1.id, "Read");
                    // write to Tier2
                    tier2_dr.exec(lfu_page_t1.id, "Write");
                }
            ...
            auto duration_LFU = std::chrono::duration_cast<std::chrono::nanoseconds>(end_time_LFU - start_time_LFU);
            // std::cout << "Descision by LFU takes " << duration_LFU.count() << " nanoseconds\n" << std::endl;
            total_runtime_LFU += duration_LFU.count(); // Accumulate duration in microseconds
        }
            ...
            Page lfu_page_t2  = LC_T2.min_heap.top();
            // skip non_existed page or non-matched LFU page
            while ((Tier2.find(lfu_page_t2.id) == Tier2.end()) ||
                   (lfu_page_t2.frequency != LC_T2.page_map[lfu_page_t2.id].frequency)) {
                LC_T2.min_heap.pop();
                lfu_page_t2  = LC_T2.min_heap.top();
            }
            // If current page has larger frequency than any page in Tier1, then upgrade current page to Tier1
            // std::cout << "Current page's frequency: " << current_page.frequency << std::endl;
            // std::cout << "LFU page in Tier2 frequency: " << lfu_page_t2.frequency << std::endl;
            if (current_page.frequency > lfu_page_t2.frequency){
                ++num_migr_t3t2;
                ...
                while (Tier2.size() > max_capacity_tier2){
                    ...
                    LC_T2.evictPage(lfu_page_t2);
                    LC_T3.addPage(lfu_page_t2);
                    // clean LC_T3.min_heap to free space
                    LC_T3.min_heap.pop();
                    // new task should be submitted here to simulate page downgrade
                    // read from Tier2
                    tier2_dr.exec(lfu_page_t2.id, "Read");
                    // write to Tier3
                    tier3_dr.exec(lfu_page_t2.id, "Write");
                }
                // new task for the migration
                // read from Tier3
        ...
        // Page lfu_page_t1  = LC_T1.min_heap.top();
        // Page lfu_page_t2  = LC_T2.min_heap.top();
        // std::cout << "LFU page in Tier1: " << lfu_page_t1.id << ", its frequency = " << lfu_page_t1.frequency <<std::endl;
        // std::cout << "LFU page in Tier2: " << lfu_page_t2.id << ", its frequency = " << lfu_page_t2.frequency <<std::endl;
    ...
    std::cout << "Total LFU runtime: " << total_runtime_LFU << " nanoseconds" << std::endl;
    ...
    std::cout << "\nAverage runtime per call: " << total_runtime_LFU/(page_hit_Tier2+page_hit_Tier3) << " nanoseconds" << std::endl;
    std::cout << "Average runtime per migration: " << total_runtime_LFU/(num_migr_t2t1+num_migr_t3t2) << " nanoseconds" << std::endl;
