...
// JSON library to read JSON: https://github.com/nlohmann/json
#include "json.hpp" 
...
// Page structure to store page id and previous accesses records for Logi prediction
...
struct Page {
    ...
    std::vector<float> Input_features;         // Input features for LR model
...
public:
    ...
    std::vector<float> weights; // Logistic regression weights
    float intercept;            // Logistic regression intercept

    LogiCache(int capacity, const std::string& model_path) : capacity(capacity) {
        loadModel(model_path); // Load the logistic regression model
    }

    void loadModel(const std::string& model_path) {
        // Load model weights and intercept from JSON file
        ...
        weights = model_json["weights"].get<std::vector<float>>();
        intercept = model_json["intercept"].get<float>();

        // std::cout << "Model loaded: " << weights.size() << " weights, intercept = " << intercept << std::endl;
    }
    ...
    void requestPage(int page_id, int current_round) {
        if (page_map.find(page_id) != page_map.end()) {
            // Page is already in cache, update last_requests and Input_features
            auto& page = page_map.at(page_id);  // Use reference to avoid multiple lookups

            // Compute Input_features before updating last_five_request_rounds
            page.Input_features.clear();
            page.Input_features.resize(5, 10000.0f);  // Initialize with 1000 (fill missing values)
            ...
            for (size_t i = 0; i < num_rounds; ++i) {
                ...
                page.Input_features[start_index + i] = static_cast<float>(gap);
            }
    ...
    float predict_access_future(int page_id) const {
        ...
        const auto& features = page.Input_features;

        if (features.size() != weights.size()) {
            std::cerr << "Feature vector size mismatch!" << std::endl;
            return -1.0f;
        }

        // Compute the dot product (w · x) + b
...
int main(
    ...
    // choose LogiReg model (which percentage of pages used for training)
    ...
    std::string log_path = folder_name + "/output_" + workload  + "_LogiReg_" + std::to_string(train_percentage) + "%.log";
    ...
    // std::string outtier_path = folder_name + "Tier123_" + workload_path + "_Logi.txt";
    ...
    // LogiReg class for Tier1, Tier2, Tier3
    std::string logi_model_path = workdir + "/ML_models/Logistic_models/logistic_model_" + workload + "_" + std::to_string(train_percentage) + "%.json";
    LogiCache LC_T1(max_capacity_tier1, logi_model_path);
    LogiCache LC_T2(max_capacity_tier2, logi_model_path);
    LogiCache LC_T3(max_capacity_tier3, logi_model_path);
    ...
    // Counter of runtime
    long long total_runtime_Logi = 0; // Variable to accumulate durations
    ...
    while (std::getline(file, line)) {
        ...
        if (it1 != Tier1.end()) {
            ...
            // std::cout << ".requestPage() takes " << duration_update.count() << " microseconds" << std::endl;
            ...
            // // Calculate the duration
            // auto duration_update = std::chrono::duration_cast<std::chrono::microseconds>(end_time_update - start_time_update);
            // std::cout << "LogiCache.requestPage() takes " << duration_update.count() << " microseconds" << std::endl;
            
            // Decide migration by Logi
            // count time
            auto start_time_Logi = std::chrono::high_resolution_clock::now();
            // Calculate the prediction on future access
            float pred_Logi = LC_T2.predict_access_future(n_page);
            ...
            // std::cout << "LogiCache.predict takes " << duration_predict.count() << " microseconds" << std::endl;
            
            // If prediction is 1 (>0.5), then upgrade current page to Tier1
            if (pred_Logi > 0.5){
                ++num_migr_t2t1;
            ...
            auto end_time_Logi = std::chrono::high_resolution_clock::now();
            // Calculate the duration
            auto duration_Logi = std::chrono::duration_cast<std::chrono::nanoseconds>(end_time_Logi - start_time_Logi);
            // std::cout << "LogiCache.predict takes " << duration_Logi.count() << " nanoseconds" << std::endl;
            total_runtime_Logi += duration_Logi.count(); // Accumulate duration in nanoseconds
        }
            ...
            auto start_time_Logi = std::chrono::high_resolution_clock::now();
            // Calculate the prediction on future access
            float pred_Logi = LC_T3.predict_access_future(n_page);
            // auto end_time_predict = std::chrono::high_resolution_clock::now();
            // // Calculate the duration
            // auto duration_predict = std::chrono::duration_cast<std::chrono::microseconds>(end_time_predict - start_time_predict);
            // std::cout << "LogiCache.predict takes " << duration_predict.count() << " microseconds" << std::endl;
            
            // If Logi prediction is 1 (>0.5), then upgrade current page to Tier2
            if (pred_Logi > 0.5){
                ++num_migr_t3t2;
    ...
    std::cout << "Total Logi runtime: " << total_runtime_Logi << " nanoseconds" << std::endl;
    ...
    std::cout << "\nAverage runtime per call: " << total_runtime_Logi/(page_hit_Tier2+page_hit_Tier3) << " nanoseconds" << std::endl;
    std::cout << "Average runtime per migration: " << total_runtime_Logi/(num_migr_t2t1+num_migr_t3t2) << " nanoseconds" << std::endl;
