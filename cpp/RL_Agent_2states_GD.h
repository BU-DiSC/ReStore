...
public:
    ...
    std::pair<double, std::vector<double>> cost_phi(const std::vector<double>& state) {
        ...
        double mu_L1 = (1 / (1 + exp( -b1 * s1 + 5e4*log(a1) )));  // use exp(log()) to prevent overflow
    ...
    std::vector<double> learn(std::vector<double> state,        //states at time t-1
                              std::vector<double> state_next,   //states at time t
                              std::vector<double> last_phi,     //phi list at time t-1
                              std::vector<double> curr_phi,     //phi list at time t
                              double cost_be,                   //v(s) at time t-1
                              double cost_af,                   //v(s) at time t
                              double reward,                    //reward R_n
                              std::vector<double> sum_phi,      //sum of phi list
                              double eta, int tau_n){           //learning rate eta for a2/b2, tau_n is the time gap (=1)
        ...
        for (size_t i = 0; i < alpha.size(); ++i) {
            ...
            //                     });
            alpha[i] = (0.1 / (1 + 20 * sum_phi[i])); //for 1e5 use 10*, can be set as a parameter
            // alpha[i] = 1e-4;
        }
